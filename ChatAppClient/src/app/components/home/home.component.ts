...
export class HomeComponent implements AfterViewChecked {
  ...
  constructor(
    ...
    this.hub = new signalR.HubConnectionBuilder().withUrl("https://localhost:7149/chat-hub").build();
  ...
  getUsers(){
    this.http.get<UserModel[]>("https://localhost:7149/api/Chats/GetUsers").subscribe(res=> {
      console.log("ss")
  ...
  changeUser(user: UserModel){
    ...
    this.http.get(`https://localhost:7149/api/Chats/GetChats?userId=${this.user.id}&toUserId=${this.selectedUserId}`).subscribe((res:any)=>{
  ...
  markAsRead(userId : string, toUserId : string) {
    ...
    this.http.post(`https://localhost:7149/api/Chats/MarkAsRead`, data).subscribe(() => {
...
getToken(): string | null {
  ...
  sendMessage(){
    ...
    this.http.post<ChatModel>("https://localhost:7149/api/Chats/SendMessage",data).subscribe
      (res=> {
        this.chats.push(res);
        this.message = "";
  ...
  sendImage() {
    ...
    if (this.file && this.selectedUserId) {
      ...
      this.http.post<ChatModel>("https://localhost:7149/api/Chats/SendImage", formData,{headers:header}).subscribe(res => {
        this.chats.push(res);
        this.file = null;
      });
    }
